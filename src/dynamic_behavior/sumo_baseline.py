...
class SUMOBaseline(SUMOBaselineBase):
    """
    ...
    def sumo_oracle(
        self, parameters: dict = {}, gui: bool = True
        ...
        # the main loop
        for t in range(self.history_length, self.sim_wallstep):
            print(f"-----------------{t}-------------------")
            ...
            for agent_id in offroad_agent_ids.difference(outofbound_agent_ids):
                if agent_attrs[agent_id].type == AgentType.VEHICLE or self.parameters["treat_ped_as_vehicle"]:
                    self.place_a_vehicle(agent_attrs[agent_id], agent_states[agent_id])
                else:
    ...
    def place_a_vehicle(self, agent_attr: AgentAttr, agent_states: list[AgentState]) -> None:
        """
    ...
    def _get_vehicle_nearest_lane(self, position: tuple[float], angle: float) -> tuple[str, int]:
        ...
        neighboring_lanes = self._get_neighboring_lanes(position, r=2)
    ...
    def get_onroad_agents(
        ...
        for v_id in exist_agent_list:
            ...
            front_bumper_pos: list[float] = list(traci.vehicle.getPosition(v_id, includeZ=True))
        ...
        for p_id in exist_ped_list:
            ...
            heading_in_waymo = wrap_radians(heading_in_sumo)
            front_bumper_pos: list[float] = list(traci.person.getPosition(p_id, includeZ=True))
            if len(front_bumper_pos) == 2:
